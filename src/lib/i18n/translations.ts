export const translations = {
	en: {
		// Navigation
		nav: {
			home: 'Home',
			game: 'Game',
			history: 'History',
			stats: 'Statistics',
			about: 'About'
		},
		// Home page
		home: {
			welcomeBack: 'Welcome back',
			description:
				'This app helps you track money during your Ocko card game. Create a new game or continue the last one.',
			startPlaying: 'Start Playing',
			createOrContinue: 'Create a new game or continue previous one',
			newGame: 'New Game',
			continue: 'Continue',
			playerNamesTip:
				'Use the same player names across games for accurate statistics. "John" and "Johnny" are tracked as different players.'
		},
		// Game page
		game: {
			currentGame: 'Current Game',
			reset: 'Reset',
			edit: 'Edit',
			bank: 'Bank',
			initial: 'Initial',
			remaining: 'Remaining',
			players: 'Players',
			noActiveGame: 'No active game',
			createNewGame: 'Create new game',
			round: 'Round',
			nextRound: 'Next Round'
		},
		// New game dialog
		newGameDialog: {
			newGame: 'New Game',
			editGame: 'Edit Game',
			setupBankAndPlayers: 'Set up bank and add players',
			editSettings: 'Edit game settings',
			bank: 'Bank',
			enterBankAmount: 'Enter bank amount',
			players: 'Players',
			playerName: 'Player name',
			noPlayersYet: 'No players added yet',
			cancel: 'Cancel',
			createGame: 'Create Game',
			updateGame: 'Update Game',
			enableMemes: 'Enable Memes',
			memesDescription: 'Show fun popups during gameplay'
		},
		// Player card
		playerCard: {
			clickToEdit: 'Click to edit'
		},
		// Meme popup
		meme: {
			bankEmpty: 'Bank is empty!',
			bankEmptyMessage: 'The bank ran out, game over.',
			playersWon: 'Players won!',
			playersWonMessage: 'Players collectively won 100€!',
			playersLost: 'Players lost!',
			playersLostMessage: 'Players collectively lost 100€!',
			playerWon: 'is on fire!',
			playerWonMessage: 'Reached 50€+ profit! Keep it going! 🔥',
			playerLost: 'is struggling!',
			playerLostMessage: 'Lost 50€+... Better luck next time! 😅',
			continue: 'Continue'
		},
		// History page
		history: {
			title: 'Game History',
			noGamesYet: 'No games yet',
			startNewGame: 'Start new game',
			bank: 'Bank',
			players: 'players',
			ended: 'Ended',
			active: 'Active'
		},
		// Stats page
		stats: {
			title: 'Statistics',
			overview: 'Overview',
			totalGames: 'Total Games',
			totalPlayers: 'Total Players',
			totalMoneyMoved: 'Total Money Moved',
			averagePerGame: 'Avg per Game',
			playerLeaderboard: 'Player Leaderboard',
			allPlayersStats: 'Statistics for all players across all games',
			player: 'Player',
			gamesPlayed: 'Games',
			balance: 'Balance',
			avgPerGame: 'Avg/Game',
			noData: 'No data yet. Play some games to see statistics.',
			rank: '#',
			balanceChart: 'Balance Overview',
			balanceChartDesc: 'Visual comparison of player balances'
		},
		// About page
		about: {
			title: 'About Ocko',
			subtitle: 'Track your scores and have fun!',
			whatIsOcko: 'What is Ocko?',
			ockoDescription:
				'Ocko is a modified blackjack-style card game where players bet against the bank. This app helps you track game progress, individual player money, and overall statistics.',
			disclaimer:
				'Note: This app is just a score tracker. We are not responsible for any financial losses during gameplay.',
			features: 'Features',
			featureList: {
				createGames: 'Create games with custom bank and players',
				quickAdjust: 'Quick value adjustments (+1, -1, +5, -5)',
				manualEdit: 'Manual score editing',
				gameHistory: 'Game history',
				statsCharts: 'Statistics and charts',
				multiDevice: 'Multi-device sync (coming soon)'
			},
			aboutCreator: 'About the Creator',
			creatorDescription: 'Ocko was created by',
			creatorName: 'Lukas Pisarcik',
			enthusiast: '- programming and gaming enthusiast.',
			supportProject: 'Support the Project',
			supportDescription: 'If you like the app, you can support its development',
			shareWinnings: 'Enjoy the winnings from Ocko? Share some of them with the creator! 😄🎉',
			supportDetails:
				'Development and maintenance of this app requires time and effort. If you want to support the project, you can contribute to the following account:',
			iban: 'IBAN',
			recipient: 'Recipient',
			message: 'Message',
			supportMessage: 'Ocko support',
			ibanNote: 'Note: Replace with actual IBAN. Every contribution is greatly appreciated!',
			techStack: 'Tech Stack',
			builtWith: 'Built with modern technologies',
			version: 'Version'
		},
		// Welcome dialog
		welcome: {
			title: 'Welcome to Ocko! 👋',
			description: 'Enter your name to get started.',
			yourName: 'Your name',
			namePlaceholder: 'Enter your name',
			continue: 'Continue',
			nameRequired: 'Please enter your name',
			saveFailed: 'Failed to save. Please try again.'
		},
		// Common
		common: {
			appName: 'Ocko',
			gameHelper: 'Game Helper',
			language: 'Language',
			theme: 'Theme',
			light: 'Light',
			dark: 'Dark',
			system: 'System',
			loading: 'Loading...'
		}
	},
	sk: {
		// Navigation
		nav: {
			home: 'Domov',
			game: 'Hra',
			history: 'Historia',
			stats: 'Statistiky',
			about: 'O aplikacii'
		},
		// Home page
		home: {
			welcomeBack: 'Vitaj spat',
			description:
				'Tato aplikacia sluzi na sledovanie penazi pocas kartovej hry Ocko. Vytvorte si novu hru alebo pokracujte v poslednej.',
			startPlaying: 'Zacni hrat',
			createOrContinue: 'Vytvor novu hru alebo pokracuj v predchadzajucej',
			newGame: 'Nova hra',
			continue: 'Pokracovat',
			playerNamesTip:
				'Pouzivaj rovnake mena hracov vo vsetkych hrach pre presne statistiky. "Jano" a "Janko" su evidovani ako rozni hraci.'
		},
		// Game page
		game: {
			currentGame: 'Aktualna hra',
			reset: 'Reset',
			edit: 'Upravit',
			bank: 'Bank',
			initial: 'Pociatocny',
			remaining: 'Zostatok',
			players: 'Hraci',
			noActiveGame: 'Ziadna aktivna hra',
			createNewGame: 'Vytvorit novu hru',
			round: 'Kolo',
			nextRound: 'Dalsie kolo'
		},
		// New game dialog
		newGameDialog: {
			newGame: 'Nova hra',
			editGame: 'Upravit hru',
			setupBankAndPlayers: 'Nastav bank a pridaj hracov',
			editSettings: 'Uprav nastavenia hry',
			bank: 'Bank',
			enterBankAmount: 'Zadaj sumu banku',
			players: 'Hraci',
			playerName: 'Meno hraca',
			noPlayersYet: 'Zatial niesu pridany ziadny hraci',
			cancel: 'Zrusit',
			createGame: 'Vytvorit hru',
			updateGame: 'Upravit hru',
			enableMemes: 'Povolit memes',
			memesDescription: 'Zobrazit zabavne okna pocas hry'
		},
		// Player card
		playerCard: {
			clickToEdit: 'Klikni pre upravu'
		},
		// Meme popup
		meme: {
			bankEmpty: 'Bank je prazdny!',
			bankEmptyMessage: 'Hra skoncila.',
			playersWon: 'Hraci vyhrali!',
			playersWonMessage: 'Hraci spolu vyhrali 100€!',
			playersLost: 'Hraci prehrali!',
			playersLostMessage: 'Hraci spolu prehrali 100€!',
			playerWon: 'je v ohni!',
			playerWonMessage: 'Dosiahol 50€+ zisk! Tak drzat! 🔥',
			playerLost: 'ma smolu!',
			playerLostMessage: 'Stratil 50€+... Snáď nabudúce! 😅',
			continue: 'Pokracovat'
		},
		// History page
		history: {
			title: 'Historia hier',
			noGamesYet: 'Zatial ziadne hry',
			startNewGame: 'Zacni novu hru',
			bank: 'Bank',
			players: 'hracov',
			ended: 'Ukoncena',
			active: 'Aktivna'
		},
		// Stats page
		stats: {
			title: 'Statistiky',
			overview: 'Prehlad',
			totalGames: 'Celkom hier',
			totalPlayers: 'Celkom hracov',
			totalMoneyMoved: 'Celkom penazi',
			averagePerGame: 'Priemer na hru',
			playerLeaderboard: 'Rebricek hracov',
			allPlayersStats: 'Statistiky vsetkych hracov zo vsetkych hier',
			player: 'Hrac',
			gamesPlayed: 'Hry',
			balance: 'Zostatok',
			avgPerGame: 'Priemer/Hra',
			noData: 'Zatial ziadne data. Zahraj niekolko hier pre zobrazenie statistik.',
			rank: '#',
			balanceChart: 'Prehlad zostatkov',
			balanceChartDesc: 'Vizualne porovnanie zostatkov hracov'
		},
		// About page
		about: {
			title: 'O aplikacii Ocko',
			subtitle: 'Pomocka pri hre Ocko - sledujte svoje vysledky a bavte sa!',
			whatIsOcko: 'Co je Ocko?',
			ockoDescription:
				'Ocko je modifikovana kartova hra podobna blackjacku, kde hraci stavaju proti banke. Tato aplikacia vam pomoze sledovat priebeh hry, peniaze jednotlivych hracov a celkove statistiky.',
			disclaimer:
				'Poznamka: Tato aplikacia je len pomocka na sledovanie skore. Nezodpovedame za ziadne financne straty pri hrani hry.',
			features: 'Funkcie',
			featureList: {
				createGames: 'Vytvaranie hier s vlastnym bankom a hracmi',
				quickAdjust: 'Rychle upravy hodnot (+1, -1, +5, -5)',
				manualEdit: 'Manualna uprava skore',
				gameHistory: 'Historia vsetkych hier',
				statsCharts: 'Statistiky a grafy',
				multiDevice: 'Synchronizacia medzi zariadeniami (pripravujeme)'
			},
			aboutCreator: 'O autorovi',
			creatorDescription: 'Ocko vytvoril',
			creatorName: 'Lukas Pisarcik',
			enthusiast: '- nadsenec do programovania a hier.',
			supportProject: 'Podpora projektu',
			supportDescription: 'Ak sa vam aplikacia paci, mozete podporit jej vyvoj',
			shareWinnings: 'Uzivas si vyhry z Ocka? Podel sa o ne s autorom! 😄🎉',
			supportDetails:
				'Vyvoj a udrzba tejto aplikacie vyzaduje cas a usilie. Ak chcete podporit projekt, mozete prispiet na nasledujuci ucet:',
			iban: 'IBAN',
			recipient: 'Meno prijemcu',
			message: 'Sprava pre prijemcu',
			supportMessage: 'Ocko podpora',
			ibanNote: 'Poznamka: Vymena uctu za skutocny IBAN. Kazdy prispevok velmi ocenim!',
			techStack: 'Technologie',
			builtWith: 'Postavene na modernych technologiach',
			version: 'Verzia'
		},
		// Welcome dialog
		welcome: {
			title: 'Vitaj v Ocko! 👋',
			description: 'Zadaj svoje meno pre zaciatok.',
			yourName: 'Tvoje meno',
			namePlaceholder: 'Zadaj svoje meno',
			continue: 'Pokracovat',
			nameRequired: 'Prosim zadaj svoje meno',
			saveFailed: 'Nepodarilo sa ulozit. Skus to znova.'
		},
		// Common
		common: {
			appName: 'Ocko',
			gameHelper: 'Pomocka pri hre',
			language: 'Jazyk',
			theme: 'Tema',
			light: 'Svetla',
			dark: 'Tmava',
			system: 'Systemova',
			loading: 'Nacitavam...'
		}
	}
} as const;

export type Language = keyof typeof translations;
export type TranslationKeys = typeof translations.en;
